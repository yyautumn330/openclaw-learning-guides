...
export class PomodoroModel {
   ...
   * 根据主题模式更新深色模式状态
   */
  private updateDarkModeFromTheme(): void {
    this.isDarkMode = (this.settings.themeMode === ThemeMode.DARK);
    console.info('PomodoroModel updateDarkModeFromTheme: themeMode=', this.settings.themeMode, ', isDarkMode=', this.isDarkMode);
  }
  ...
  async setThemeMode(mode: ThemeMode): Promise<void> {
    ...
    this.updateDarkModeFromTheme();
    if (this.preferencesInstance) {
      await this.preferencesInstance.put('themeMode', mode);
      await this.preferencesInstance.flush();
      console.info('PomodoroModel setThemeMode: themeMode saved=', mode, ', isDarkMode=', this.isDarkMode);
    }
    // 发送主题变更事件（立即同步到所有页面）
    eventBus.emitThemeChanged(this.isDarkMode);
    console.info('PomodoroModel setThemeMode: event emitted, isDarkMode=', this.isDarkMode);
  }
  ...
  async reloadSettings(): Promise<void> {
    ...
    try {
      ...
      const loadedTheme = await this.preferencesInstance.get('themeMode', ThemeMode.LIGHT) as ThemeMode;
      const themeMode = (loadedTheme === ThemeMode.DARK) ? ThemeMode.DARK : ThemeMode.LIGHT;
  ...
  async resetSettings(): Promise<void> {
    const currentThemeMode = this.settings.themeMode;
    this.settings = {
      pomodoroDuration: 25,
      shortBreakDuration: 5,
      longBreakDuration: 15,
    ...
    this.achievements.forEach(achievement => {
      ...
      if (shouldUnlock) {
        ...
        import('../utils/EventBus').then(module => {
          module.eventBus.emitAchievementUnlocked(achievement.name);
        });
      }
    });
  ...
  async saveToPreferences(): Promise<void> {
    ...
    try {
      ...
      // 保存主题模式设置
      await this.preferencesInstance.put('themeMode', this.settings.themeMode);
  ...
  async loadFromPreferences(): Promise<void> {
    ...
    try {
      ...
      // 加载主题模式
      const loadedTheme = await this.preferencesInstance.get('themeMode', ThemeMode.LIGHT) as ThemeMode;
      this.settings.themeMode = (loadedTheme === ThemeMode.DARK) ? ThemeMode.DARK : ThemeMode.LIGHT;
